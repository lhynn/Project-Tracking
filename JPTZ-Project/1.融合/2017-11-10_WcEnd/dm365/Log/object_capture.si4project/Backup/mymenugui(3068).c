...
void SaveInfoToFile(void){
	...
	sprintf(log_arr,"%d %d %d %d %d %d %d",LIANGDUCOUNT,DUIBIDUCOUNT,FANGDACOUNT,\
		JIXINGCOUNT,FENHUACOUNT,TUQIANGCOUNT,WEICAICOUNT);
	write(log_fd,log_arr,25);
	ret = fsync(log_fd);
	if(ret < 0){
		printf("--->fsync error.\n");
		return ;
...
static void InitMenuThree_CaiDan(unsigned char*ptr){	
	...
	if(CAIDANSELCETFLAG == 7)
		osd_print_rect(610,530,GREENPAN,ptr);
	else
		osd_print_rect(610,530,0,ptr);
	osd_print_Chinese(622, 532,YELLOWPAN,ptr,CAISE,2,4);
}
...
static void InitMenuFour_WC(unsigned char*ptr){
	...
	osd_print_jindutiaoRect(200,4,274,530, YELLOWPAN,ptr);
	osd_print_jindutiaoRect(6,20,274+(WEICAICOUNT*5/6), 522, YELLOWPAN,ptr);	
	osd_print_jindutiaoRect( (WEICAICOUNT*5/6),4,274, 530, GREENPAN,ptr);
	/*
	if( WEICAICOUNT == 240)
		sprintf(&dbdstr[1], "%d", WEICAICOUNT*100/240);
	else
		sprintf(&dbdstr[1], "%0.2d", WEICAICOUNT*100/240);
...
static PT_MenuGuiSt DealMenuFour_WC(PT_MenuGuiSt ptr){
	...
	if(glUsartInfoSt.RightRoll){
		WEICAICOUNT += 16;
		if(WEICAICOUNT > 240) WEICAICOUNT = 240;
		SaveInfoToFile();
		glUsartInfoSt.RightRoll = 0;
	}
	if(glUsartInfoSt.LeftRoll){
		WEICAICOUNT -= 16;
		if(WEICAICOUNT < 0) WEICAICOUNT = 0;
		SaveInfoToFile();	
		glUsartInfoSt.LeftRoll = 0;
	}
	if(glUsartInfoSt.LeftKeyInfo == 1){
...
static void InsertChildMenuHead(PT_MenuGuiSt FarthMenu,int num,MYINITFUN initfun,MYDEALFUN dealfun){
	...
	if(num == 7)
		FarthMenu->ChildMenu7 = MenuChild;
}
...
void CreateSecMenu(PT_MenuGuiSt Head){
	...
	InsertChildMenuHead(Head->ChildMenu1->ChildMenu4,7,InitMenuFour_WC,DealMenuFour_WC);
}
