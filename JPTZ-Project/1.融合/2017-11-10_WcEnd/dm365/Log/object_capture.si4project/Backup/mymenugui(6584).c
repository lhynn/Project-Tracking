...
static void CreatTuPianShow(int num,unsigned char*ptr,PT_MyFileList head,int Select){
	int i;
	PT_MyFileList tmp = head;
	for(i = 0;i < num ;i++){
		osd_print_rect(250+100*i+(4-num)*100,490,0,ptr);
		if(i == (Select - 1))
			osd_print_string(tmp->name,5,254+100*i+(4-num)*100,486,GREENPAN,ptr,MYASCIIDOT);
		else
			osd_print_string(tmp->name,5,254+100*i+(4-num)*100,486,0xff,ptr,MYASCIIDOT);
		
		tmp = tmp->prev; 
	}
}	
static int gl_maxfilecnt;
static PT_MyFileList ImageTmpList = NULL;
...
static void InitMenuThree_TuPian(unsigned char*ptr){
	...
	if(LinkCreateFlag == 0){
		...
		ImageTmpList   = headImage->prev;
		gl_maxfilecnt  = myGetFileListMemberSum(headImage);
	...
	if(gl_maxfilecnt < 4){
		CreatTuPianShow(gl_maxfilecnt,ptr,ImageTmpList,PICTURESELCETFLAG);
	}else{
		CreatTuPianShow(4,ptr,ImageTmpList,PICTURESELCETFLAG);
	}
...
static PT_MenuGuiSt DealMenuThree_TuPian(PT_MenuGuiSt ptr){
	...
	if(glUsartInfoSt.RightRoll){
		...
		if(PICTURESELCETFLAG < 1){
				PICTURESELCETFLAG = 1;
			if(ImageTmpList->next == headImage){
				ImageTmpList = ImageTmpList->next->next;
			}else{
				ImageTmpList = ImageTmpList->next;
			}
		}
		glUsartInfoSt.RightRoll = 0;
	}
