...
#include "../ctrl.h"
int osd_print_GuangMin_off(int start_x, int start_y, unsigned char* bufptr)
{    
	int offset;
...
int osd_print_GuangMin_on(int start_x, int start_y, unsigned char* bufptr)
{    
	int offset;
...
{    
	...
	if(gl_light&0x01)
		osd_print_GuangMin_on(start_x,start_y,bufptr);
	else
		osd_print_GuangMin_off(start_x,start_y,bufptr);
    ...
    for (j=0; j<48; j++)
    {
        for (k=0; k<48; k++)  //icon width
        {
            ...
            {*(bufptr + offset) = 50; 
					...
					*(bufptr + uvstart + vvoffset) = 10;//128;//200; 	//set 
			}
