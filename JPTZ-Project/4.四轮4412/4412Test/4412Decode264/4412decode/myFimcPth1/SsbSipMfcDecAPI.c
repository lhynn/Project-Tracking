...
static int isPBPacked(_MFCLIB *pCtx, int Frameleng)
{
    ...
    while (1) {
        ...
        do {
            if (*strmBuffer == 'p') {
                printf(">> peter strmBuffer = 0x%08x <<\n", *strmBuffer);
...
void *SsbSipMfcDecOpen(void)
{
    ...
    if (access(mfc_dev_name, F_OK) != 0) {
        //printf("SsbSipMfcDecOpen] MFC device node not exists");
        free(pCTX);
        return NULL;
    }
...
void *SsbSipMfcDecOpenExt(void *value)
{
    ...
    pCTX = (_MFCLIB *)malloc(sizeof(_MFCLIB));
    if (pCTX == NULL) {
        //printf("SsbSipMfcDecOpenExt] malloc failed.\n");
        return NULL;
    }
    memset(pCTX, 0, sizeof(_MFCLIB));
    ...
    hMFCOpen = open(mfc_dev_name, O_RDWR | O_NDELAY);
    if (hMFCOpen < 0) {
        //printf("SsbSipMfcDecOpenExt] MFC Open failure");
        free(pCTX);
        return NULL;
    }
    ...
    if ((err < 0) || (CommonArg.ret_code != MFC_OK)) {
        //printf("SsbSipMfcDecOpenExt] IOCTL_MFC_SET_BUF_CACHE failed");
        free(pCTX);
        close(hMFCOpen);
        return NULL;
    }
    ...
    mapped_size = ioctl(hMFCOpen, IOCTL_MFC_GET_MMAP_SIZE, &CommonArg);
    if ((mapped_size < 0) || (CommonArg.ret_code != MFC_OK)) {
        //printf("SsbSipMfcDecOpenExt] IOCTL_MFC_GET_MMAP_SIZE failed");
        free(pCTX);
        close(hMFCOpen);
        return NULL;
    }
    ...
    mapped_addr = (unsigned int)mmap(0, mapped_size, PROT_READ | PROT_WRITE, MAP_SHARED, hMFCOpen, 0);
    if (!mapped_addr) {
        //printf("SsbSipMfcDecOpenExt] FIMV5.x driver address mapping failed");
        free(pCTX);
        close(hMFCOpen);
        return NULL;
    }
...
SSBSIP_MFC_ERROR_CODE SsbSipMfcDecInit (void *openHandle, SSBSIP_MFC_CODEC_TYPE codec_type, int Frameleng)
{
    ...
    if (openHandle == NULL) {
        printf("SsbSipMfcDecInit] openHandle is NULL");
    ...
    if ((codec_type != MPEG4_DEC)  &&
        ...
        (codec_type != VC1_DEC)) {
        printf("SsbSipMfcDecInit] Undefined codec type");
        return MFC_RET_INVALID_PARAM;
    }
    ...
    if (DecArg.ret_code != MFC_OK) {
        printf("SsbSipMfcDecInit] IOCTL_MFC_DEC_INIT failed\n");
...
SSBSIP_MFC_ERROR_CODE SsbSipMfcDecExe(void *openHandle, int lengthBufFill)
{
    ...
    if ((lengthBufFill < 0) || (lengthBufFill > MAX_DECODER_INPUT_BUFFER_SIZE)) {
        //printf("SsbSipMfcDecExe] lengthBufFill is invalid. (lengthBufFill=%d)", lengthBufFill);
        return MFC_RET_INVALID_PARAM;
    }
    ...
    if (framecount != 0) {
        if (mDec2.tv_sec == mDec1.tv_sec)
            //printf("SsbSipMfcDecExe] Interval between IOCTL_MFC_DEC_EXE's (end to start) = %8d", (mDec1.tv_usec - mDec2.tv_usec));
        else
            //printf("SsbSipMfcDecExe] Interval between IOCTL_MFC_DEC_EXE's (end to start) = %8d", (1000000 + (mDec1.tv_usec - mDec2.tv_usec)));
    }
#endif
#endif
    ...
    if (DecArg.ret_code != MFC_OK) {
        //printf("SsbSipMfcDecExe] IOCTL_MFC_DEC_EXE failed(ret : %d)", DecArg.ret_code);
    ...
    if (mDec1.tv_sec == mDec2.tv_sec) {
        if (mDec2.tv_usec - mDec1.tv_usec > 30000)
            over30ms++;
#ifdef CONFIG_MFC_PERF_//LOG
        //printf("SsbSipMfcDecExe] Time consumed for IOCTL_MFC_DEC_EXE = %8d", ((mDec2.tv_usec - mDec1.tv_usec)));
#endif
    } else {
        if (1000000 + mDec2.tv_usec - mDec1.tv_usec > 30000)
            over30ms++;
#ifdef CONFIG_MFC_PERF_//LOG
        //printf("SsbSipMfcDecExe] Time consumed for IOCTL_MFC_DEC_EXE = %8d", (1000000 + (mDec2.tv_usec - mDec1.tv_usec)));
#endif
    }
    ...
    if (DecArg.args.dec_exe.out_display_status == 4) {
        //printf("SsbSipMfcDecExe] Resolution is chagned");
        /*
...
#ifdef CONFIG_MFC_FPS
    //printf(">>> Statistics in MFC API:");
    //printf(">>> Total number of IOCTL_MFC_DEC_EXE = %d", framecount);
    //printf(">>> Number of IOCTL_MFC_DEC_EXE taking more than 30ms = %d", over30ms);
    //printf(">>> Avg IOCTL_MFC_DEC_EXE time = %dsec %.2fmsec", (int)mAvg.tv_sec, (float)(mAvg.tv_usec / 1000.0));
#endif
...
void  *SsbSipMfcDecGetInBuf(void *openHandle, void **phyInBuf, int inputBufferSize)
{
    ...
    if (inputBufferSize < 0) {
        //printf("SsbSipMfcDecGetInBuf] inputBufferSize = %d is invalid", inputBufferSize);
        return NULL;
    }
    ...
    if (ret_code < 0) {
        //printf("SsbSipMfcDecGetInBuf] IOCTL_MFC_GET_PHYS_ADDR failed");
        return NULL;
    }
...
SSBSIP_MFC_ERROR_CODE SsbSipMfcDecSetConfig(void *openHandle, SSBSIP_MFC_DEC_CONF conf_type, void *value)
{
    ...
    if (DecArg.ret_code != MFC_OK) {
        //printf("SsbSipMfcDecSetConfig] IOCTL_MFC_SET_CONFIG failed(ret : %d, conf_type: 0x%08x)", DecArg.ret_code, conf_type);
...
SSBSIP_MFC_ERROR_CODE SsbSipMfcDecGetConfig(void *openHandle, SSBSIP_MFC_DEC_CONF conf_type, void *value)
{
    ...
    */
    ...
    case MFC_DEC_GETCONF_PHYS_ADDR:
        ...
        if (r < 0) {
            //printf("SsbSipMfcDecGetConfig] IOCTL_MFC_GET_PHYS_ADDR failed");
            return MFC_API_FAIL;
        }
    ...
    case MFC_DEC_GETCONF_FRAME_PACKING:
        ...
        if (DecArg.ret_code != MFC_OK) {
            //printf("SsbSipMfcDecGetConfig] IOCTL_MFC_GET_CONFIG failed(ret : %d, conf_type: 0x%08x)", DecArg.ret_code, conf_type);
