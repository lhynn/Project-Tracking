...
/*
	...
	if(-1 == ioctl(iFd, VIDIOC_S_PARM, &Stream_Parm)){
		printf("------------------------->error.\n");
		return -1;
	}*/
	...
	if(-1 == ioctl(iFd,VIDIOC_S_PARM, setfps)){
		printf("------------------------->error.\n");
		return -1;
	}
	ioctl(iFd,VIDIOC_G_PARM, setfps);
	...
	for (i = 0; i < ptVideoDevice->iVideoBufCnt; i++) {
		...
		ptVideoDevice->pucVideoBuf[i] = (unsigned char *)mmap (NULL , buf.length, PROT_READ | PROT_WRITE ,\
