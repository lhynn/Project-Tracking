...
void SaveInfoToFile(void){
	...
	sprintf(log_arr,"%d %d %d %d %d %d %d %d %d",LIANGDUCOUNT,DUIBIDUCOUNT,FANGDACOUNT,\
		JIXINGCOUNT,FENHUACOUNT,TUQIANGCOUNT,BIAOJICOUNT,WEICAICOUNT,GUANGGANCOUNT);
	write(log_fd,log_arr,30);
	ret = fsync(log_fd);
	if(ret < 0){
		printf("--->fsync error.\n");
		return ;
...
void ShowPower(unsigned char *ptr){
	...
	sprintf(batstr, "%d%\r\n", batval);
	osd_print_string(batstr, 4, 644, 24,0xff,ptr,MYASCIIDOT);  //printf battery value width = 4*6=24 FENHUA
	osd_print_battery(678, 26, batval, ptr,BATTICON);
	
	if(FENHUACOUNT == 2)
		osd_print_cross(347, 270, ptr,SDCARDFULL,CROSS);

	osd_print_jindutiaoRect(80,20,58,23,0,ptr);
	...
	osd_print_24("T",1,690,60,0xff,ptr);
	osd_print_jindutiaoRect(3,90, 695,105,0xff,ptr);
	osd_print_24("W",1,690,218,0xff,ptr);
	osd_print_Mypic(24,0xff,689,89,ptr,SANJIAO);	
	osd_print_Mypic(24,0xff,689,190,ptr,DAOSANJIAO);
	//CheckUsartData();
/*
	osd_print_hollow_Rect(iMarGinSt.MgLenX,iMarGinSt.MgLenY,iMarGinSt.LastOffersetX - iMarGinSt.MgLenX,\
						iMarGinSt.LastOffersetY - iMarGinSt.MgLenY,GREENPAN,ptr);	*/
}
...
static void InitMenuHead(unsigned char*ptr){
	...
	osd_print_Mypic(24,0xff,686,275,ptr,MYPHOTO);
...
static void InitMenuSec(unsigned char*ptr){
	...
	osd_print_jindutiaoRect(40,20,260,546,0,ptr);
	...
	osd_print_jindutiaoRect(40,20,320,546,0,ptr);
	...
	osd_print_jindutiaoRect(40,20,380,546,0,ptr);
	...
	osd_print_jindutiaoRect(40,20,440,546,0,ptr);
...
static PT_MenuGuiSt DealMenuSec(PT_MenuGuiSt ptr){
	...
	if(glUsartInfoSt.RightKeyInfo != 0){
		if(GREENPICFLAG & 0x01)
			pt_retnode = ptr->ChildMenu1;
		if(GREENPICFLAG & 0x02){
...
static PT_MenuGuiSt DealMenuThree_MoShi(PT_MenuGuiSt ptr){
	...
	if(glUsartInfoSt.RightKeyInfo != 0){
		...
		if(MOSHISELECTFLAG == 3){
			if(WEICAICOUNT)	WEICAICOUNT = 0;
			else WEICAICOUNT = 1;
			if(WEICAICOUNT)BIAOJICOUNT = 0;
			SaveInfoToFile();
		}
		if(MOSHISELECTFLAG == 4)
			pt_retnode = ptr->ChildMenu1;
...
static void InitMenuThree_CaiDan(unsigned char*ptr){	
	...
	if(CAIDANSELCETFLAG == 1)
		osd_print_rect(140,530,YELLOWPAN,ptr);
	else
		osd_print_rect(140,530,0,ptr);
	osd_print_Chinese(152, 532,YELLOWPAN,ptr,LIANGDU,2,4);

	if(CAIDANSELCETFLAG == 2)
		osd_print_rect(220,530,GREENPAN,ptr);
	else
		osd_print_rect(220,530,0,ptr);
	osd_print_Chinese(222,532,YELLOWPAN,ptr,DUIBIDU,3,4);

	if(CAIDANSELCETFLAG == 3)
		osd_print_rect(300,530,GREENPAN,ptr);
	else
		osd_print_rect(300,530,0,ptr);
	osd_print_Chinese(312, 532,YELLOWPAN,ptr,FANGDA,2,4);

	if(CAIDANSELCETFLAG == 4)
		osd_print_rect(380,530,GREENPAN,ptr);
	else
		osd_print_rect(380,530,0,ptr);
	osd_print_Chinese(393, 532,YELLOWPAN,ptr,FENHUA,2,4);

	if(CAIDANSELCETFLAG == 5)
		osd_print_rect(460,530,GREENPAN,ptr);
	else
		osd_print_rect(460,530,0,ptr);
	osd_print_Chinese(472, 532,YELLOWPAN,ptr,GUANGGAN,2,4);

	if(CAIDANSELCETFLAG == 6)
		osd_print_rect(540,530,GREENPAN,ptr);
	else
		osd_print_rect(540,530,0,ptr);
	osd_print_Chinese(540, 532,YELLOWPAN,ptr,TUXIANGZHENGQIANG,4,-1);
}
...
static void InitMenuFour_LD(unsigned char*ptr){
	...
	osd_print_Mypic(24,YELLOWPAN,214,520,ptr,LIANGDUJINDUTIAO1);
	...
	osd_print_24("-", 1,254,520,YELLOWPAN,ptr);
	osd_print_24("+", 1,484,520,YELLOWPAN,ptr);
	/*
	ldstr[0] = '%';
	if( LIANGDUCOUNT == 240)
...
static void InitMenuFour_DBD(unsigned char*ptr){
	...
	osd_print_Mypic(24,YELLOWPAN,214,520,ptr,DUIBIDUJINDUTIAO);
	osd_print_jindutiaoRect(200,4,274,530, YELLOWPAN,ptr);
...
static void InitMenuFour_BJ(unsigned char*ptr){
	ShowRECTime(ptr);
	...
	osd_print_jindutiaoRect(200,4,274,530, YELLOWPAN,ptr);
	osd_print_jindutiaoRect(6,20,274+(BIAOJICOUNT*5/6), 522, YELLOWPAN,ptr);	
	osd_print_jindutiaoRect( (BIAOJICOUNT*5/6),4,274, 530, GREENPAN,ptr);

	osd_print_24("-", 1,254,520,YELLOWPAN,ptr);
	osd_print_24("+", 1,484,520,YELLOWPAN,ptr);
}
static PT_MenuGuiSt DealMenuFour_BJ(PT_MenuGuiSt ptr){
	PT_MenuGuiSt pt_retnode = NULL;
	//if(glUsartInfoSt.RightKeyInfo)
	//if(glUsartInfoSt.LeftKeyInfo)
	if(glUsartInfoSt.RightRoll){
		BIAOJICOUNT += 16;
		if(BIAOJICOUNT > 240) BIAOJICOUNT = 240;
		SaveInfoToFile();
		glUsartInfoSt.RightRoll = 0;
	}
	if(glUsartInfoSt.LeftRoll){
		BIAOJICOUNT -= 16;
		if(BIAOJICOUNT < 0) BIAOJICOUNT = 0;
		SaveInfoToFile();	
		glUsartInfoSt.LeftRoll = 0;
	}
	if(glUsartInfoSt.LeftKeyInfo == 1){
...
static void InitMenuFour_GG(unsigned char*ptr){
	ShowRECTime(ptr);
	osd_print_Mypic(40,0,20,268,ptr,MYHEISEYUAN);
	osd_print_Mypic(24,0xff,28,276,ptr,MYRETURN);
...
static PT_MenuGuiSt DealMenuFour_GG(PT_MenuGuiSt ptr){
	PT_MenuGuiSt pt_retnode = NULL;
	//if(glUsartInfoSt.RightKeyInfo)
	//if(glUsartInfoSt.LeftKeyInfo)
	if(glUsartInfoSt.RightRoll){
...
void CreateSecMenu(PT_MenuGuiSt Head){
	...
	InsertChildMenuHead(Head->ChildMenu1->ChildMenu4,4,InitMenuFour_FH,DealMenuFour_FH);
	InsertChildMenuHead(Head->ChildMenu1->ChildMenu4,5,InitMenuFour_GG,DealMenuFour_GG);
	InsertChildMenuHead(Head->ChildMenu1->ChildMenu4,6,InitMenuFour_TQ,DealMenuFour_TQ);

	InsertChildMenuHead(Head->ChildMenu1->ChildMenu1,1,InitMenuFour_BJ,DealMenuFour_BJ);
}
