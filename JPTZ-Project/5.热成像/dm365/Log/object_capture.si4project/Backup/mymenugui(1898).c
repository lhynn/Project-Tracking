...
static int  JIXINGFLAG        = 0;
static int  JIXINGCOUNT       = 0;
...
void ShowPower(unsigned char *ptr){
	...
	char fangda[2];
	sprintf(batstr, "%d%\r\n", batval);
	osd_print_string(batstr, 4, 624, 25,0xff,ptr,MYASCIIDOT);  //printf battery value width = 4*6=24 FENHUA
	osd_print_battery(658, 26, batval, ptr,BATTICON);
	
	if(FENHUACOUNT == 2)
		osd_print_cross(344, 264, ptr,SDCARDFULL,CROSS);

	sprintf(fangda,"%dX",FANGDAFLAG+1);
	...
	osd_print_24("T",1,660,85,0xff,ptr);
	osd_print_jindutiaoRect(3,90, 664,130,0xff,ptr);
	osd_print_24("W",1,659,226,0xff,ptr);
	osd_print_Mypic(16,0xff,657,114,ptr,SANJIAO);
...
static void InitMenuSec(unsigned char*ptr){
	...
	osd_print_Chinese(262, 497,YELLOWPAN,ptr,MOSHI,2,4);
	...
	osd_print_Chinese(322, 497,YELLOWPAN,ptr,TUPIAN,2,4);
	
	if(GREENPICFLAG & 0x04)
		osd_print_Mypic(40,GREENPAN,380,450,ptr,MYHEISEYUAN);
	else
		osd_print_Mypic(40,0,380,450,ptr,MYHEISEYUAN);
	osd_print_Mypic(24,0xff,390,458,ptr,MYSHIPIN);
	osd_print_Chinese(382, 497,YELLOWPAN,ptr,SHIPIN,2,4);
	...
	osd_print_Chinese(442, 497,YELLOWPAN,ptr,CAIDAN,2,4);
}
...
static void InitMenuThree_CaiDan(unsigned char*ptr){	
	...
	if(CAIDANSELCETFLAG == 1)
		osd_print_rect(130,450,YELLOWPAN,ptr);
	else
		osd_print_rect(130,450,0,ptr);
	osd_print_Chinese(142, 452,YELLOWPAN,ptr,LIANGDU,2,4);

	if(CAIDANSELCETFLAG == 2)
		osd_print_rect(210,450,GREENPAN,ptr);
	else
		osd_print_rect(210,450,0,ptr);
	osd_print_Chinese(212,452,YELLOWPAN,ptr,DUIBIDU,3,4);

	if(CAIDANSELCETFLAG == 3)
		osd_print_rect(290,450,GREENPAN,ptr);
	else
		osd_print_rect(290,450,0,ptr);
	osd_print_Chinese(302, 452,YELLOWPAN,ptr,FANGDA,2,4);

	if(CAIDANSELCETFLAG == 4)
		osd_print_rect(370,450,GREENPAN,ptr);
	else 
		osd_print_rect(370,450,0,ptr);
	osd_print_Chinese(382, 452,YELLOWPAN,ptr,JIXING,2,4);
	
	if(CAIDANSELCETFLAG == 5)
		osd_print_rect(450,450,GREENPAN,ptr);
	else
		osd_print_rect(450,450,0,ptr);
	osd_print_Chinese(462, 452,YELLOWPAN,ptr,FENHUA,2,4);

	if(CAIDANSELCETFLAG == 6)
		osd_print_rect(530,450,GREENPAN,ptr);
	else
		osd_print_rect(530,450,0,ptr);
	osd_print_Chinese(530, 452,YELLOWPAN,ptr,TUXIANGZHENGQIANG,4,-1);
}
...
static void InitMenuFour_LD(unsigned char*ptr){
	...
	osd_print_Mypic(24,YELLOWPAN,244,440,ptr,LIANGDUJINDUTIAO1);
	osd_print_jindutiaoRect(200,4,274,450,YELLOWPAN,ptr);	
	osd_print_jindutiaoRect(6,20,234+LIANGDUCOUNT, 442, YELLOWPAN,ptr);
	osd_print_jindutiaoRect(LIANGDUCOUNT-40,4,274, 450, GREENPAN,ptr);
	...
	osd_print_string(ldstr,4,484,445,YELLOWPAN,ptr,MYASCIIDOT);  //printf battery value width = 4*6=24
}
...
static void InitMenuFour_DBD(unsigned char*ptr){
	...
	osd_print_Mypic(24,YELLOWPAN,244,440,ptr,DUIBIDUJINDUTIAO);
	osd_print_jindutiaoRect(200,4,274,450, YELLOWPAN,ptr);
	osd_print_jindutiaoRect(6,20,234+DUIBIDUCOUNT, 442, YELLOWPAN,ptr);	
	osd_print_jindutiaoRect(DUIBIDUCOUNT-40,4,274, 450, GREENPAN,ptr);
	...
	osd_print_string(dbdstr, 4, 484,445,YELLOWPAN,ptr,MYASCIIDOT); 
}
...
static void InitMenuFour_FD(unsigned char*ptr){
	...
	osd_print_string("1X",2,304,452,YELLOWPAN,ptr,MYASCIIDOT);
	
	if(FANGDAFLAG == 1)
		osd_print_rect(360,450,GREENPAN,ptr);
	else
		osd_print_rect(360,450,0,ptr);
	osd_print_string("2X",2,384,452,YELLOWPAN,ptr,MYASCIIDOT);
...
static void InitMenuFour_JX(unsigned char*ptr){
	...
	if(!JIXINGFLAG)
		...
		osd_print_rect(290,450,0xff,ptr);
	osd_print_Chinese(302, 452,YELLOWPAN,ptr,BAIRE,2,4);

	if(JIXINGFLAG)
		osd_print_rect(370,450,GREENPAN,ptr);
	else
		osd_print_rect(370,450,0xff,ptr);
	osd_print_Chinese(382, 452,YELLOWPAN,ptr,HEIRE,2,4);
}
...
static void InitMenuFour_FH(unsigned char*ptr){
	...
	if(FENHUAFLAG == 1)
		...
		osd_print_rect(290,450,0xff,ptr);
	osd_print_Chinese(302, 452,YELLOWPAN,ptr,GUANBI,2,4);

	if(FENHUAFLAG == 2)
		osd_print_rect(370,450,GREENPAN,ptr);
	else
		osd_print_rect(370,450,0xff,ptr);
	osd_print_Chinese(382, 452,YELLOWPAN,ptr,DAKAI,2,4);
}
