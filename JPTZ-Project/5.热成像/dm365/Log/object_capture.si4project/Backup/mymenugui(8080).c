...
void OneFristFuZhi(void){
	...
	int ret;
ret= ioctl(gl_capture_fd, VPFE_CMD_OLED_CONTRAST,DUIBIDUCOUNT);
	//  while (ret < 0);
ret= ioctl(gl_capture_fd, VPFE_CMD_OLED_BRIGHT_DOWN,LIANGDUCOUNT);
//while (ret < 0);
...
void SaveInfoToFile(void){
	...
	sprintf(log_arr,"%d %d %d %d %d %d %d %d",LIANGDUCOUNT,DUIBIDUCOUNT,FANGDACOUNT,\
...
static void InitMenuThree_CaiDan(unsigned char*ptr){	
	...
	if(CAIDANSELCETFLAG == 1)
		osd_print_rect(180,530,YELLOWPAN,ptr);
	else
		osd_print_rect(180,530,0,ptr);
	osd_print_Chinese(192, 532,YELLOWPAN,ptr,LIANGDU,2,4);

	if(CAIDANSELCETFLAG == 2)
		osd_print_rect(260,530,GREENPAN,ptr);
	else
		osd_print_rect(260,530,0,ptr);
	osd_print_Chinese(262,532,YELLOWPAN,ptr,DUIBIDU,3,4);

	if(CAIDANSELCETFLAG == 3)
		osd_print_rect(340,530,GREENPAN,ptr);
	else
		osd_print_rect(340,530,0,ptr);
	osd_print_Chinese(352, 532,YELLOWPAN,ptr,FANGDA,2,4);

	if(CAIDANSELCETFLAG == 4)
		osd_print_rect(420,530,GREENPAN,ptr);
	else
		osd_print_rect(420,530,0,ptr);
	osd_print_Chinese(433, 532,YELLOWPAN,ptr,FENHUA,2,4);

	if(CAIDANSELCETFLAG == 5)
		osd_print_rect(500,530,GREENPAN,ptr);
	else
		osd_print_rect(500,530,0,ptr);
	osd_print_Chinese(500, 532,YELLOWPAN,ptr,TUXIANGZHENGQIANG,4,-1);
}
...
void CreateSecMenu(PT_MenuGuiSt Head){
	...
	InsertChildMenuHead(Head->ChildMenu1->ChildMenu4,5,InitMenuFour_TQ,DealMenuFour_TQ);
