...
//    pView->setGeometry(969,7,30,30);//1019
  ...
  //  ShowPhoto->setPixmap(QPixmap("/Icon_TranRot/IconShow/HelpRead.png"));
   ...
   // i3DPixShow->setStyleSheet("background-color: rgb(0, 0, 255);border:8px solid gray;border-color:rgb(60,60,60);");
    ...
    connect( timer2, SIGNAL(timeout()), this, SLOT(timerDecodeTimer()));
    timer2->start(500); // 2秒单触发定时器

    timer3 = new QTimer(this);
    connect( timer2, SIGNAL(timeout()), this, SLOT(timerFanZhuan()));
    timer3->start(1000); // 2秒单触发定时器
}
//
...
void se_terminal::timerFanZhuan(void){
    if(g_iCarStateInfo.CheTiFanZhuan)
      msgServer.sendCmd(QtInterface_Hide);
    else
      msgServer.sendCmd(QtInterface_Dec);
}

void se_terminal::timerDecodeTimer(void){
    system("sync");
}
...
int se_terminal::IsFullFile(char *filename,int type){
    int ret;
    char strName[40];
    if(type == IMAGE_TYPE)
       sprintf(strName,"/home/root/media/%s.jpg",filename);
    else
       sprintf(strName,"/home/root/media/%s.avi",filename);
    QFile file(strName);
...
void se_terminal::ShowPictureToView(void){
    if(!IsFullFile(SecMenuList->item(SeclistFlag)->text().toLatin1().data(),IMAGE_TYPE)){
    ...
    case MENUSTATESEC:
        ...
        case 0:                                          //视频和图片项
            if(SecMenuList->count()){
                if(!IsFullFile(SecMenuList->item(SeclistFlag)->text().toLatin1().data(),AVI_TYPE))
                    if(PressOutTimer){
                      ShowHelpPic->hide();
...
void  se_terminal::OsdUpdateFunc(int RetSt){
    ...
    if(MenuCurState == MENUSTATEHIDE){
        ...
        if(g_iCarStateInfo.BaiBiRot == 360) g_iCarStateInfo.BaiBiRot = 0;
