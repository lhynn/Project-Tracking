...
**
**************************************************************************************************/
int DM368PutH264DataToBuffer(unsigned char* vidbuf, unsigned int vlength, unsigned char frametype)
{
//	unsigned int i,j;
//	unsigned int len=0,off=0;//,len2=2;
	unsigned char *pstr;
	int iframe=0;

    if(n<NMAX)
    {
		memcpy(ringfifo[iput].buffer, vidbuf, vlength);
		pstr = vidbuf;
