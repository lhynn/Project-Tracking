...
#define DECODE_DISP_WIDTH    800
#define DECODE_DISP_HEIGHT   576
...
{    
	...
	int uvstart = DECODE_DISP_WIDTH*DECODE_DISP_HEIGHT;
	int uuoffset;
	int vvoffset;
        ...
        for (k=0; k<BATT_ICON_WIDTH; k++)
        {
			...
			if( (buffer & 0x80)  == 0x80 )
			{
				*(bufptr + offset) = 0xff;  //Y channel
				...
				uuoffset = (y/2)*DECODE_DISP_WIDTH + (x/2)*2;
				vvoffset = (y/2)*DECODE_DISP_WIDTH + (x/2)*2 + 1;
    ...
    for (i=0; i<len; i++)   //draw a character
    {
        ...
        for (j = 0; j < MINI_CHAR_HEIGHT; j++)
        {
            ...
            for (k = 0; k < MINI_CHAR_WIDTH; k++)   //draw a line
            {
                ...
                offset = x + y*DECODE_DISP_WIDTH;      //  y*vScreenInfo.xres;
