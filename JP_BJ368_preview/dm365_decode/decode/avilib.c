...
int avi_parse_input_file(avi_t *AVI, int getIndex)
{
  ...
  /* Read first 12 bytes and check that this is an AVI file */
   ...
   while(1)
   {
      ...
      if(strncasecmp(data,"LIST",4) == 0)
      {
	    ...
	    header_offset = lseek(AVI->fdes,0,SEEK_CUR);
            ...
            if( avi_read(AVI->fdes,(char *)hdrl_data,n) != (size_t)(n) ) ERR_EXIT(AVI_ERR_READ)
         }
      ...
      else if(strncasecmp(data,"idx1",4) == 0)
      {
         ...
         if(avi_read(AVI->fdes, (char *) AVI->idx, n) != (size_t)(n) ) ERR_EXIT(AVI_ERR_READ)
      }
      else
         lseek(AVI->fdes,n,SEEK_CUR);
   }
   ...
   for(i=0;i<hdrl_len;)
   {
      ...
      else if(strncasecmp((char *)hdrl_data+i,"strf",4)==0)
      {
	    ...
	    char *nwfe;
            ...
            if ((size_t)(hdrl_len - i) < sizeof(WAVEFORMATEX_avilib))
              wfes = hdrl_len - i;
            else
   ...
   if(AVI->idx)
   {
      ...
      if( strncasecmp(data,(char *)AVI->idx[i],4)==0 && str2ulong((unsigned char *)data+4)== (unsigned long)(len) )
      {
...
long AVI_read_frame(avi_t *AVI, char *vidbuf, int *keyframe)
{
   ...
   *keyframe = (AVI->video_index[AVI->video_pos].key==0x10) ? 1:0;
   ...
   if (avi_read(AVI->fdes,vidbuf,n) != (size_t)(n) )
   {
...
long AVI_read_audio(avi_t *AVI, char *audbuf, long bytes)
{
   ...
   while(bytes>0)
   {
      ...
      lseek(AVI->fdes, pos, SEEK_SET);
      if (avi_read(AVI->fdes,audbuf+nr,todo) != (size_t)(todo) )
      {
         AVI_errno = AVI_ERR_READ;
         return -1;
      }
...
long AVI_read_audio_chunk(avi_t *AVI, char *audbuf)
{
   ...
   pos = AVI->track[AVI->aptr].audio_index[AVI->track[AVI->aptr].audio_posc].pos + AVI->track[AVI->aptr].audio_posb;
   lseek(AVI->fdes, pos, SEEK_SET);
   if (avi_read(AVI->fdes,audbuf,left) != (size_t)(left) )
   {
      AVI_errno = AVI_ERR_READ;
      return -1;
   }
   ...
   while(1)
   {
      ...
      if(strncasecmp(data,AVI->video_tag,3) == 0)
      {
         *len = n;
         ...
         if(avi_read(AVI->fdes,vidbuf,n) != (size_t)(n) ) return 0;
         return 1;
      }
      else if(strncasecmp(data,AVI->track[AVI->aptr].audio_tag,4) == 0)
      {
         *len = n;
         ...
         if(avi_read(AVI->fdes,audbuf,n) != (size_t)(n) ) return 0;
         return 2;
         break;
      }
      else
